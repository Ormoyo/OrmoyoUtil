...
public class MessageGetAbilitySyncedValueOnClient extends AbstractMessage<MessageGetAbilitySyncedValueOnClient> {
	...
	public MessageGetAbilitySyncedValueOnClient() {
	}
	
	public MessageGetAbilitySyncedValueOnClient(@Nonnull Ability ability, @Nonnull String fieldName, UUID id, EntityPlayer player) {
	...
	public MessageGetAbilitySyncedValueOnClient(@Nonnull Ability ability, @Nullable Class<?> superClass, @Nonnull String fieldName, UUID id, EntityPlayer player) {
		this.ability = ability.getRegistryName();
		this.name = fieldName;
	...
	@Override
	public void onClientReceived(Minecraft client, MessageGetAbilitySyncedValueOnClient message, EntityPlayer player, MessageContext messageContext) {
		Set<Ability> set = OrmoyoUtil.proxy.getUnlockedAbilities(player);
		System.out.println(message.ability);
			...
			if(ability.getRegistryName().equals(message.ability)) {
				if(message.superClass == null) {
					try {
						...
						OrmoyoUtil.NETWORK_WRAPPER.sendToServer(new MessageInvokeGetSyncedValueEventOnServer(player, ability, field.get(ability), message.id));
	...
	@Override
	public void onServerReceived(MinecraftServer server, MessageGetAbilitySyncedValueOnClient message, EntityPlayer player, MessageContext messageContext) {
	}
}

...
public class MessageUpdateAbilitySyncedValueOnClient extends AbstractMessage<MessageUpdateAbilitySyncedValueOnClient> {
	...
	public MessageUpdateAbilitySyncedValueOnClient() {
	}
	
	public MessageUpdateAbilitySyncedValueOnClient(@Nonnull Ability ability, @Nonnull String fieldName, @Nullable Object value, EntityPlayer player) {
	...
	public MessageUpdateAbilitySyncedValueOnClient(@Nonnull Ability ability, @Nullable Class<?> superClass, @Nonnull String fieldName, @Nullable Object value, EntityPlayer player) {
		this.ability = ability.getRegistryName();
		this.name = fieldName;
		this.value = value;
	...
	@Override
	public void onClientReceived(Minecraft client, MessageUpdateAbilitySyncedValueOnClient message, EntityPlayer player, MessageContext messageContext) {
			...
			if(ability.getRegistryName().equals(message.ability)) {
				if(message.superClass == null) {
					try {
						...
						if(field.isAnnotationPresent(AbilitySyncedValue.OnlyChangableForClient.class) || !field.isAnnotationPresent(AbilitySyncedValue.ChangeableValue.class) && !field.isAnnotationPresent(AbilitySyncedValue.OnlyChangableForServer.class)) return;
	...
	@Override
	public void onServerReceived(MinecraftServer server, MessageUpdateAbilitySyncedValueOnClient message, EntityPlayer player, MessageContext messageContext) {
	}
}

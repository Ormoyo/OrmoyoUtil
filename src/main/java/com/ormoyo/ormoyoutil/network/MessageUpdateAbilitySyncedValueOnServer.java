...
public class MessageUpdateAbilitySyncedValueOnServer extends AbstractMessage<MessageUpdateAbilitySyncedValueOnServer> {
	...
	public MessageUpdateAbilitySyncedValueOnServer() {
	}
	
	public MessageUpdateAbilitySyncedValueOnServer(@Nonnull Ability ability, @Nonnull String fieldName, @Nullable Object value, EntityPlayer player) {
	...
	public MessageUpdateAbilitySyncedValueOnServer(@Nonnull Ability ability, @Nullable Class<?> superClass, @Nonnull String fieldName, @Nullable Object value, EntityPlayer player) {
		this.ability = ability.getRegistryName();
		this.name = fieldName;
		this.value = value;
	...
	@Override
	public void onClientReceived(Minecraft client, MessageUpdateAbilitySyncedValueOnServer message, EntityPlayer player, MessageContext messageContext) {
	}

	@Override
	public void onServerReceived(MinecraftServer server, MessageUpdateAbilitySyncedValueOnServer message, EntityPlayer player, MessageContext messageContext) {
			...
			if(ability.getRegistryName().equals(message.ability)) {
				if(message.superClass == null) {
					try {
						...
						if(field.isAnnotationPresent(AbilitySyncedValue.OnlyChangableForServer.class) || !field.isAnnotationPresent(AbilitySyncedValue.ChangeableValue.class) && !field.isAnnotationPresent(AbilitySyncedValue.OnlyChangableForClient.class)) return;

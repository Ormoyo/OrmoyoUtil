...
public class MessageUpdateAbilitySyncedValueToServer extends AbstractMessage<MessageUpdateAbilitySyncedValueToServer> {
	...
	public MessageUpdateAbilitySyncedValueToServer() {
	}
	
	public MessageUpdateAbilitySyncedValueToServer(@Nonnull Ability ability, @Nonnull String fieldName, @Nullable Object value, EntityPlayer player) {
	...
	public MessageUpdateAbilitySyncedValueToServer(@Nonnull Ability ability, @Nullable Class<?> superClass, @Nonnull String fieldName, @Nullable Object value, EntityPlayer player) {
		this.ability = ability.getEntry().getRegistryName();
		this.name = fieldName;
		this.value = value;
	...
	@Override
	public void onClientReceived(Minecraft client, MessageUpdateAbilitySyncedValueToServer message, EntityPlayer player, MessageContext messageContext) {
	}

	@Override
	public void onServerReceived(MinecraftServer server, MessageUpdateAbilitySyncedValueToServer message, EntityPlayer player, MessageContext messageContext) {
			...
			if(ability.getEntry().getRegistryName().equals(message.ability)) {
				if(message.superClass == null) {
					try {
						...
						if(field.isAnnotationPresent(AbilitySyncedValue.OnlyChangableForServer.class) || !field.isAnnotationPresent(AbilitySyncedValue.ChangeableValue.class)) return;

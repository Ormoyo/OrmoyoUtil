...
public class MessageInvokeGetSyncedValueEventOnServer extends AbstractMessage<MessageInvokeGetSyncedValueEventOnServer> {
	EntityPlayer player;
	ResourceLocation ability;
	Object value;
	UUID id;
	public MessageInvokeGetSyncedValueEventOnServer() {
	}
	
	public MessageInvokeGetSyncedValueEventOnServer(EntityPlayer player, Ability ability, Object value, UUID id) {
		this.player = player;
	...
	@Override
	public void onClientReceived(Minecraft client, MessageInvokeGetSyncedValueEventOnServer message, EntityPlayer player, MessageContext messageContext) {
	}

	@Override
	public void onServerReceived(MinecraftServer server, MessageInvokeGetSyncedValueEventOnServer message, EntityPlayer player, MessageContext messageContext) {
		...
		for(Ability ability : set) {
			if(ability.getRegistryName().equals(message.ability)) {
				...
				MinecraftForge.EVENT_BUS.post(new AbilityEvent.AbilityGetSyncedValueEvent(ability, message.value, message.id));
			}
		}
	}
}
